...
class ResourceMinifiedComponent {
  ...
  void add(ResourceResponse templateUri) {
    if(!_newResourceService.request.templateData.contains(templateUri.name)) {
      _newResourceService.request.templateData.add(templateUri.name);
    }
  }
  ...
  bool isAdded() {
    return isInEdit() &&
        _newResourceService.request.templateData.contains(model.name);
  }
